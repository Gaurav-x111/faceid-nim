...
//! sends.
//!
//! POLKIT IS STUBBED. `require_polkit` below returns Ok(()) with a
//! TODO. It is called at the top of every mutating method, so wiring
//! `org.freedesktop.PolicyKit1.Authority.CheckAuthorization` is a
//! one-function change with no other seams. Until that lands, Enroll
//! is authenticated by uid but not by password, which means someone at
//! an unlocked laptop could add their own face. Do not ship without it.
//!
...
impl Session {
    ...
    fn push_preview(&mut self, jpeg: &[u8]) {
        let Some(file) = self.preview.as_mut() else { return };
        if jpeg.len() > u32::MAX as usize {
            return;
        }
        let len = (jpeg.len() as u32).to_be_bytes();
        if file.write_all(&len).is_err() || file.write_all(jpeg).is_err() {
...
impl Enrollment1 {
    pub fn new(cfg: Arc<Mutex<Config>>, store: Arc<Store>,
               worker: Arc<Worker>, conn: Connection) -> Self {
        Self {
            cfg,
            store,
            worker,
...
/// TODO wire CheckAuthorization.
///
/// This is the single seam for polkit. Replace the body with a call to
/// org.freedesktop.PolicyKit1.Authority.CheckAuthorization using the
/// caller's bus name as the subject and `action` as the action id, and
/// return AccessDenied when it is not authorized. Nothing else in this
...
impl Enrollment1 {
    ...
    async fn start_enrollment(
        ...
        require_polkit(&self.conn, &hdr, "org.faceidnim.enroll").await?;
        ...
        let (read_fd, write_fd) = make_pipe()
            .map_err(|e| zbus::fdo::Error::Failed(format!("preview pipe: {e}")))?;
        ...
        let pump = tokio::spawn(async move {
            while let Some(ev) = prx.recv().await {
                let (status, p) = match ev {
                    ...
                    Progress::Progress(p) => (ST_GOOD, p as f64),
                    Progress::Challenge(_) => (ST_ACQUIRING, 0.1),
                };
                let _ = Enrollment1::enroll_progress(
                    &sig_ctxt, &sig_session, idx, pose_name, status, p,
                )
                .await;
            }
        });
            ...
            .with_session(&session, uid, |s| {
                ...
                for e in &ev.embeddings {
                    ...
                    let dup = s
                        .embeddings
                        .iter()
                        .any(|t| crate::matcher::cosine_max(e, std::slice::from_ref(t)) > DUP_SIMILARITY);
                    if !dup {
    ...
    async fn finish_enrollment(
        ...
        let _ = Self::enroll_progress(
            &ctxt, &session, s.pose_index, "", ST_FINISHED, 1.0,
        )
        .await;
        Ok(n)
    }
...
///
/// Called by whatever reads worker events (see worker.rs); kept here so
/// the only code that can write to a preview pipe lives in this file.
impl Enrollment1 {
    pub async fn deliver_preview(&self, session: &str, uid: u32, jpeg: &[u8]) {
        let mut map = self.sessions.lock().await;
        let Some(s) = map.get_mut(session) else { return };
