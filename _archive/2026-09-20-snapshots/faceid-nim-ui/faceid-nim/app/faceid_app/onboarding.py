...
class OnboardingWindow(Adw.Window):
    ...
    def _enroll_page(self) -> Gtk.Widget:
        ...
        self.pose_label.add_css_class("faceid-pose-name")
        box.append(self.pose_label)

        self.pose_counter = Gtk.Label(label="1 of 9")
        self.pose_counter.add_css_class("faceid-pose-counter")
        box.append(self.pose_counter)

        # Rounded, clipped preview. A raw square feed reads as a debug
        # window; the rounded frame is most of what makes this look
        # like a product rather than a test harness.
        frame = Gtk.Frame(halign=Gtk.Align.CENTER)
        frame.add_css_class("faceid-preview")
        frame.set_overflow(Gtk.Overflow.HIDDEN)
        self.picture = Gtk.Picture(content_fit=Gtk.ContentFit.COVER,
                                   width_request=340, height_request=260)
        self.picture.set_can_shrink(True)
        frame.set_child(self.picture)
        box.append(frame)

        # One dot per pose, filled as each is captured. Gives the user
        # a sense of how much is left that a bare bar does not.
        self.dots = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=7,
    ...
    def _rebuild_dots(self) -> None:
        ...
        for i in range(len(self.poses)):
            ...
            if i < self.pose_index:
                dot.add_css_class("faceid-dot-ok")       # captured
            elif i == self.pose_index:
                dot.add_css_class("faceid-dot-warn")     # in progress
