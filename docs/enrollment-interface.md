...
Bind polkit to the member that writes. `StartEnrollment`,
`EnrollPose`, `GetSessionInfo`, `ListPoses`, and `CancelEnrollment`
are deliberately ungated: opening the camera and abandoning a session
write nothing to disk, and requiring a password to see the scanner
would break enrollment when no auth agent is reachable.

| Member | Signature | Polkit action | Used by the app for |
|---|---|---|---|
| `ListPoses` | `() → (as)` | — | Pose prompts, so app and daemon cannot drift |
| `StartEnrollment` | `(s identity) → (s session, h preview_fd)` | — (camera preview only; writes nothing) | Opens the session, returns the preview pipe |
...
`daemon/src/authz.rs` has one seam:

```rust
pub async fn require_polkit(conn, hdr, action) -> zbus::fdo::Result<()>
```

It calls PolicyKit `CheckAuthorization` and fails closed. It is called
at the top of `FinishEnrollment` before anything is persisted, so
enrollment requires both the correct uid and a password re-auth.
