...
class FaceGlyph extends St.DrawingArea {
    _init() {
        ...
        this._ramp = 0;
        this._failed = false;
        this._face = 'apple';       // 'apple' (default) | 'arena' | 'classic'
        this.connect('repaint', a => this._redraw(a));
    }

    /* Select which scan face to draw. Set by the pill from the user's
     * config each scan: 'apple' (default iPhone-like) | 'arena' | 'classic'. */
    setScanFace(face) {
        this._face = (face === 'classic' || face === 'arena') ? face : 'apple';
    }
    ...
    _redraw(area) {
        ...
        if (this._face === 'apple') {
            if (this._phase === 'matched')
                this._drawAppleMatched(cr, cx, cy, S, thm);
            else
                this._drawAppleRing(cr, cx, cy, S, thm);
            cr.$dispose();
            return;
        }
    ...
    _drawRing(cr, cx, cy, S, col) {
        ...
        if (glowIn > 0.01) {
            ...
            grad.addColorStopRGBA(0, gc[0], gc[1], gc[2], 0.16 * glowIn);
            grad.addColorStopRGBA(1, gc[0], gc[1], gc[2], 0);
            cr.setSource(grad);
            cr.paint();
        }
    ...
    /* ---- Apple Face ID face (default) -------------------------------
     * iPhone-like: thin circular track, white progress arc that fills
     * with `progress`/sweep, centred face outline with smiling eyes +
     * smile that draws in as the scan locks on. No comet/spiral/beam —
     * calm, single-purpose, instantly recognisable as Face ID.
     * Success: ring turns Apple green, face fades, check draws + pops. */
    _drawAppleRing(cr, cx, cy, S, col) {
        const R = S * 0.40;
        const trackW = Math.max(2, S * 0.045);
        const progW = Math.max(2.5, S * 0.058);
        const sweep = this._scanT * TAU;
        // Progress-driven fill: verifying raises floor, scanning sweeps.
        const fill = Math.max(this._progress, 0.0);
        const arcLen = TAU * (0.18 + 0.82 * fill);

        // Soft glow behind.
        if (!this._failed && this._ramp > 0.01) {
            const grad = new Cairo.RadialGradient(cx, cy, 0, cx, cy, S * 0.7);
            grad.addColorStopRGBA(0, SCAN_CYAN[0], SCAN_CYAN[1], SCAN_CYAN[2], 0.12 * this._ramp);
            grad.addColorStopRGBA(1, SCAN_CYAN[0], SCAN_CYAN[1], SCAN_CYAN[2], 0);
            cr.setSource(grad);
            cr.paint();
        }

        // Track: faint full circle.
        this._alpha(cr, SCAN_WHITE, 0.16 * this._ramp);
        cr.setLineWidth(trackW);
        cr.newPath();
        cr.arc(cx, cy, R, 0, TAU);
        cr.stroke();

        // Progress arc: white-hot head, rotates while scanning.
        if (!this._failed) {
            this._alpha(cr, SCAN_WHITE, 0.95 * this._ramp);
            cr.setLineWidth(progW);
            cr.newPath();
            cr.arc(cx, cy, R, sweep - arcLen, sweep);
            cr.stroke();
            // Head dot.
            this._alpha(cr, SCAN_WHITE, 0.9 * this._ramp);
            cr.newPath();
            cr.arc(cx + R * Math.cos(sweep), cy + R * Math.sin(sweep),
                   Math.max(1.6, S * 0.032), 0, TAU);
            cr.fill();
        }

        // Apple face: rounded head outline + happy eyes + smile.
        // Smile draws in with fill so lock-on is visible.
        const fade = this._failed ? 1 : this._ramp;
        if (fade > 0.02)
            this._drawAppleFace(cr, cx, cy, S, fade, fill);

        if (this._failed) {
            this._alpha(cr, col, 0.9);
            cr.setLineWidth(Math.max(2, S * 0.06));
            const arm = S * 0.16;
            cr.moveTo(cx - arm, cy - arm);
            cr.lineTo(cx + arm, cy + arm);
    ...
    _drawAppleFace(cr, cx, cy, S, fade, fill) {
        // Head: simple rounded outline, iPhone proportions.
        const fw = S * 0.30, fh = S * 0.36;
        const y0 = cy - fh / 2;
        this._alpha(cr, SCAN_WHITE, 0.95 * fade);
        cr.setLineWidth(Math.max(1.6, S * 0.030));
        cr.newPath();
        cr.moveTo(cx - fw / 2, y0 + S * 0.02);
        cr.lineTo(cx - fw / 2, y0 + fh - fw / 2);
        cr.arc(cx, y0 + fh - fw / 2, fw / 2, Math.PI, 0);
        cr.lineTo(cx + fw / 2, y0 + S * 0.02);
        cr.stroke();
        // Smiling eyes: short upward arcs (not dots) — the Apple look.
        this._alpha(cr, SCAN_WHITE, 0.9 * fade);
        cr.setLineWidth(Math.max(1.4, S * 0.024));
        const ew = S * 0.055, ey = y0 + S * 0.155;
        for (const ex of [cx - S * 0.075, cx + S * 0.075]) {
            cr.newPath();
            cr.arc(ex, ey + ew * 0.35, ew, Math.PI * 1.15, Math.PI * 1.85);
            cr.stroke();
        }
        // Smile: draws in with fill (lock-on feedback).
        const s = clamp01(0.25 + 0.75 * fill);
        if (s > 0.02) {
            this._alpha(cr, SCAN_WHITE, 0.9 * fade);
            cr.setLineWidth(Math.max(1.4, S * 0.026));
            cr.newPath();
            cr.arc(cx, y0 + S * 0.20, S * 0.075, Math.PI * (0.15 + 0.35 * (1 - s)),
                   Math.PI * (0.85 - 0.35 * (1 - s)));
            cr.stroke();
        }
    }

    _drawAppleMatched(cr, cx, cy, S, col) {
        const green = SUCCESS_GREEN;
        const R = S * 0.40;
        // Green ring draws closed fast, then holds.
        const c = easeOutCubic(clamp01(this._t / 0.30));
        this._alpha(cr, SCAN_WHITE, 0.16 * (1 - c));
        cr.setLineWidth(Math.max(2, S * 0.045));
        cr.newPath();
        cr.arc(cx, cy, R, 0, TAU);
        cr.stroke();
        this._alpha(cr, green, 0.95);
        cr.setLineWidth(Math.max(2.5, S * 0.058));
        cr.newPath();
        cr.arc(cx, cy, R, -Math.PI / 2, -Math.PI / 2 + TAU * Math.max(c, 0.001));
        cr.stroke();

        // Halo + glow pulse.
        const halo = easeOutCubic(clamp01((this._t - 0.25) / 0.14));
        if (halo > 0) {
            const grad = new Cairo.RadialGradient(cx, cy, S * 0.05, cx, cy, S * 0.60);
            grad.addColorStopRGBA(0, green[0], green[1], green[2], 0.26 * halo);
            grad.addColorStopRGBA(1, green[0], green[1], green[2], 0);
            cr.setSource(grad);
            cr.paint();
        }

        // Check draws + pops (same language as arena matched).
        const s1 = easeOutCubic(clamp01((this._t - 0.32) / 0.22));
        const s2 = easeOutCubic(clamp01((this._t - 0.54) / 0.22));
        if (s1 > 0) {
            const pop = easeOutBack(clamp01(this._t / 0.28));
            const k = 0.82 + 0.18 * pop;
            cr.save();
            cr.translate(cx, cy);
            cr.scale(k, k);
            cr.translate(-cx, -cy);
            const p0 = [S * 0.28, S * 0.53];
            const p1 = [S * 0.44, S * 0.69];
            const p2 = [S * 0.74, S * 0.33];
    ...
    _drawGlow(cr, cx, cy, S, col) {
        ...
        grad.addColorStopRGBA(0, col[0], col[1], col[2], 0.30 * g);
        grad.addColorStopRGBA(1, col[0], col[1], col[2], 0);
        cr.setSource(grad);
        cr.paint();
    }
