...
class ScanConfig:
    ...
    # Active camera mode from the request ("rgb" | "ir" | "both"). The
    # worker gates RGB-specific screen cues (glare/bezel/moire) on it:
    # those FFT- and contrast-based checks are tuned for webcam texture
    # statistics and false-positive on IR sensor noise. IR screens are
    # detected by analyse_ir_face()'s "screen_dark" cue instead.
    mode: str = "rgb"
...
class ScanEngine:
    ...
    def scan(self, camera: Camera, cfg: ScanConfig,
        ...
        try:
            for _ts, frame in camera.frames(cfg.timeout_ms / 1000.0):
                ...
                self._collect_deny_cues(frame, face, state, cfg, ir_camera)
    ...
    def _collect_deny_cues(self, frame, face: Face, state: LivenessState,
        ...
        # must never promote an "rgb" scan to IR analysis.
        if cfg.mode in ("ir", "both") and ir_camera is not None:
